...
export const VideoModeSelector: React.FC<VideoModeSelectorProps> = ({ currentMode, onSelect }) => {
    const modes = [
        ...
        { mode: 'FIRST_LAST_FRAME' as const, icon: Link, title: '首尾插帧 (FrameWeaver)' },
        ...
        <div className="flex items-center gap-1">
            {modes.map(item => (
                <button 
                   ...
                   className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md transition-all text-[10px] font-bold border ${currentMode === item.mode ? 'bg-white text-black border-white shadow-md' : 'bg-white/70 border-slate-300 text-slate-600 hover:text-slate-900 hover:border-white/30 backdrop-blur-md opacity-70 hover:opacity-100'}`}
                   title={item.title}
                >
                    <item.icon size={12} />
