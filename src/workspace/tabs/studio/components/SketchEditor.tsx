...
export const SketchEditor: React.FC<SketchEditorProps> = ({ onClose, onGenerate }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-50 flex flex-col animate-in fade-in duration-300">
            ...
            <div className="h-14 border-b border-slate-300 flex items-center justify-between px-6 bg-white">
                <button 
                    onClick={onClose}
                    className="absolute left-6 p-2 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 transition-colors"
                >
                    <X size={16} />
                </button>
            ...
            <div className="flex-1 relative bg-[#121214] flex items-center justify-center p-8 overflow-hidden">
                ...
                {/* Floating Toolbar */}
                <div className="absolute top-12 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 p-1.5 bg-[#ffffff]/90 backdrop-blur-xl border border-slate-300 rounded-full shadow-2xl">
                    <button 
                        onClick={() => setTool('brush')}
                        className={`p-2.5 rounded-full transition-colors ${tool === 'brush' ? 'bg-blue-500 text-black' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}
                        title="画笔"
                    >
                        <Brush size={16} />
                    </button>
                    ...
                    <button 
                        ...
                        className={`p-2.5 rounded-full transition-colors ${tool === 'eraser' ? 'bg-blue-500 text-black' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}
                        title="橡皮擦"
                    >
                        <Eraser size={16} />
                    </button>
                    ...
                    <div className="relative">
                        <button 
                            ...
                            className="p-2.5 rounded-full transition-colors text-slate-600 hover:text-slate-900 hover:bg-slate-50 relative"
                            title="调色板"
                        >
                            <Palette size={16} style={{ color: tool === 'brush' ? brushColor : undefined }} />
                            <div className="absolute bottom-1 right-1 w-2 h-2 rounded-full border border-[#ffffff]" style={{ backgroundColor: brushColor }} />
                        </button>

                        {showPalette && (
                            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 p-3 bg-white border border-slate-300 rounded-xl shadow-xl grid grid-cols-4 gap-2 w-48 z-30">
                                {PRESET_COLORS.map(c => (
                                    <button 
                                        key={c}
                    ...
                    <button onClick={handleUndo} className="p-2.5 rounded-full text-slate-600 hover:text-slate-900 hover:bg-slate-50">
                        <Undo size={16} />
                    </button>
                ...
                {/* The Canvas Wrapper */}
                <div className="relative shadow-2xl rounded-lg overflow-hidden border border-slate-200 bg-[#ffffff] select-none" style={{ aspectRatio: '16/9', height: '100%', maxHeight: '800px' }}>
            ...
            <div className="h-20 bg-white border-t border-slate-300 flex items-center px-8 gap-4">
                {/* Tools (Left) */}
                <div className="flex items-center gap-2 mr-4">
                    {/* Import Background Button */}
                     <div 
                        className="relative p-2 rounded-lg bg-slate-50 text-slate-600 hover:text-slate-900 border border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors"
                    ...
                    <button onClick={() => { if(canvasRef.current){ const a = document.createElement('a'); a.href = getCompositeDataURL(); a.download='sketch.png'; a.click(); } }} className="p-2 rounded-lg bg-slate-50 text-slate-600 hover:text-slate-900 border border-slate-200" title="下载当前画布">
                        <Download size={16} />
                    </button>
                </div>
                ...
                <div className="flex-1 relative">
                    <input 
                        ...
                        className="w-full h-11 bg-slate-900/30 border border-slate-300 rounded-xl px-4 text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:border-blue-500/50 transition-colors"
                ...
                <div className="flex items-center gap-3">
                    <div className="h-11 px-4 flex items-center gap-2 bg-slate-900/30 border border-slate-300 rounded-xl text-xs text-slate-600 font-medium">
                    ...
                    <button 
                        ...
                        className={`
                            ...
                            ${isGenerating || !prompt.trim() 
                                ...
                                : activeMode === 'pose' 
                                    ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-900 hover:scale-105' 
                                    : 'bg-gradient-to-r from-cyan-600 to-blue-600 text-slate-900 hover:scale-105'}
                        `}
                    >
