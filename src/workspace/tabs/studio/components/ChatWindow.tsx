          ...
          <div key={i} className={`flex w-full ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex flex-col max-w-[92%] gap-1.5 ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className="flex items-center gap-2 px-1">
                    {m.role === 'model' && <span className="text-[10px] font-bold text-blue-500/80 uppercase tracking-wider">SunStudio AI</span>}
                ...
                <div className="group relative transition-all">
                    <div 
                        className={`
                            ...
                            ${m.role === 'user' 
                                ...
                                : 'bg-white border-slate-200 text-slate-600 rounded-tl-sm w-full'
                            }
                        `}
                    >
                    ...
                    <button 
                        ...
                        className={`absolute top-2 ${m.role === 'user' ? '-left-8' : '-right-8'} p-1.5 rounded-full bg-white/70 border border-slate-300 text-slate-600 opacity-0 group-hover:opacity-100 transition-all hover:text-slate-900 hover:scale-110 hover:bg-white/90`}
                        title="复制内容"
                    >
        ...
        {isLoading && (
            <div className="flex justify-start w-full animate-in fade-in slide-in-from-bottom-2">
                <div className="flex flex-col gap-2 max-w-[85%]">
                    <span className="text-[10px] font-bold text-blue-500/80 uppercase tracking-wider px-1">Thinking</span>
                    <div className="px-5 py-4 bg-white border border-slate-200 rounded-2xl rounded-tl-sm flex items-center gap-3 w-fit shadow-lg shadow-cyan-900/10">
                        <Loader2 size={16} className="animate-spin text-blue-500" />
                        <span className="text-xs text-slate-600 font-medium tracking-wide">正在思考创意...</span>
                    </div>
                </div>
            </div>
        )}
