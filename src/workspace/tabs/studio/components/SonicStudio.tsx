...
export const SonicStudio: React.FC<SonicStudioProps> = ({ isOpen, onClose, history, onGenerate }) => {
    ...
    return (
        <div 
            className={`fixed inset-0 z-[100] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] bg-slate-50 overflow-hidden flex ${isOpen ? 'opacity-100 scale-100 translate-x-0' : 'opacity-0 scale-95 -translate-x-10 pointer-events-none'}`}
            style={{ transformOrigin: 'left center' }}
        >
            {/* --- Background Ambience --- */}
            <div className="absolute top-[-20%] right-[-10%] w-[600px] h-[600px] rounded-full bg-cyan-200/30 blur-[120px] pointer-events-none" />
            <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-purple-200/30 blur-[100px] pointer-events-none" />

            {/* --- Left Sidebar (Navigation) --- */}
            <div className="w-64 h-full border-r border-slate-200 bg-white/95 backdrop-blur-sm flex flex-col z-10 shadow-[15px_0_45px_rgba(15,23,42,0.05)]">
                <div className="h-16 flex items-center px-6 border-b border-slate-200 gap-3">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-900/20">
                        <AudioLines size={16} className="text-slate-900" />
                    </div>
                    <span className="text-sm font-bold tracking-wide text-slate-900">Audio Hub</span>
                </div>
                
                <div className="flex flex-col gap-1 p-4">
                    <button 
                        onClick={() => setActiveTab('factory')}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'factory' ? 'bg-slate-100 text-slate-900 shadow-md' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                    >
                        <Mic2 size={16} /> 声音工厂 (Voice Factory)
                    </button>
                    <button 
                        onClick={() => setActiveTab('transcribe')}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'transcribe' ? 'bg-slate-100 text-slate-900 shadow-md' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                    >
                        <FileText size={16} /> 语音转文字 (Transcriber)
                    </button>
                    <button 
                        onClick={() => setActiveTab('live')}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'live' ? 'bg-slate-100 text-slate-900 shadow-md' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                    >
                        ...
                        {history.length === 0 ? (
                            ...
                            history.map((item) => (
                                <div 
                                    ...
                                    className={`group flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all ${currentTrack?.id === item.id ? 'bg-slate-100' : 'hover:bg-slate-50'}`}
                                >
                                    <div className={`w-6 h-6 rounded bg-gradient-to-br ${getRandomCover(item.id)} flex items-center justify-center shrink-0`}>
                                        <Mic2 size={10} className="text-slate-700" />
                                    </div>
                                    <span className={`text-xs truncate ${currentTrack?.id === item.id ? 'text-blue-400' : 'text-slate-600'}`}>{item.title}</span>
                                </div>
                            ))
                        )}
                    </div>
            ...
            <div className="flex-1 flex flex-col relative z-0">
                ...
                <div className="absolute top-6 left-6 flex items-center gap-4 z-20">
                    <button onClick={onClose} className="p-2 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 transition-colors">
                        <X size={20} />
                    </button>
                </div>
                        ...
                        {activeTab === 'factory' && (
                            <>
                                <div className="space-y-1 pl-12"> {/* Added left padding for title due to close button */}
                                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">声音工厂 <span className="text-slate-500 font-light ml-2 text-xl">Voice Factory</span></h1>
                                    <p className="text-slate-600 text-sm">Create realistic voiceovers, clone voices, and generate emotional speech.</p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    <div className="lg:col-span-1 flex flex-col gap-6">
                                        <div className="space-y-3">
                                            ...
                                            <div 
                                                className={`relative h-32 rounded-xl border border-dashed transition-all flex flex-col items-center justify-center gap-2 cursor-pointer group overflow-hidden ${referenceAudio ? 'border-blue-500/50 bg-blue-500/5' : 'border-slate-300 hover:border-white/30 bg-slate-50 hover:bg-slate-100'}`}
                                                onClick={() => fileInputRef.current?.click()}
                                            >
                                                <input type="file" ref={fileInputRef} className="hidden" accept="audio/*" onChange={handleUploadSample} />
                                                {referenceAudio ? (
                                                    <>
                                                        <div className="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400"><FileAudio size={20} /></div>
                                                        <span className="text-xs font-medium text-blue-200 truncate max-w-[80%]">{referenceFileName}</span>
                                                        <button onClick={(e) => { e.stopPropagation(); setReferenceAudio(null); setReferenceFileName(null); }} className="absolute top-2 right-2 p-1.5 rounded-full bg-white/70 hover:bg-red-500/80 text-white/50 hover:text-slate-900 transition-colors"><X size={10} /></button>
                                                    </>
                                                ) : (
                                                    <>
                                                        <Mic2 size={24} className="text-slate-500 group-hover:text-blue-400 transition-colors" />
                                                        <span className="text-xs text-slate-500 group-hover:text-slate-600">点击上传参考音频</span>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                {EMOTIONS.map(emo => (
                                                    <button key={emo.value} onClick={() => setSelectedEmotion(emo)} className={`px-3 py-2 rounded-lg text-xs font-medium text-left transition-all ${selectedEmotion.value === emo.value ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-slate-50 text-slate-600 hover:bg-slate-100 border border-transparent'}`}>
                                                        {emo.label}
                                                    </button>
                                                ))}
                                            </div>
                                    ...
                                    <div className="lg:col-span-2 flex flex-col gap-6">
                                        <div className="relative group flex-1">
                                            ...
                                            <div className="relative bg-white border border-slate-300 rounded-2xl p-5 shadow-2xl flex flex-col h-full min-h-[240px]">
                                                <textarea className="w-full flex-1 bg-transparent text-lg text-slate-700 placeholder-slate-600 focus:outline-none resize-none font-medium leading-relaxed" placeholder="在此输入您想生成的语音文本..." value={textPrompt} onChange={(e) => setTextPrompt(e.target.value)} />
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                {VOICE_PERSONAS.map((persona) => (
                                                    <button key={persona.label} onClick={() => { setSelectedPersona(persona); setReferenceAudio(null); setReferenceFileName(null); }} className={`p-3 rounded-xl border text-left transition-all group ${selectedPersona?.label === persona.label ? 'bg-purple-500/10 border-purple-500/50' : 'bg-slate-50 border-slate-200 hover:border-slate-400 hover:bg-slate-100'}`}>
                                                        <div className="flex justify-between items-start mb-1"><span className={`text-xs font-bold ${selectedPersona?.label === persona.label ? 'text-purple-400' : 'text-slate-600'}`}>{persona.label}</span></div>
                        ...
                        {activeTab === 'transcribe' && (
                            <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pl-12">
                                <div className="space-y-1">
                                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">语音转文字 <span className="text-slate-500 font-light ml-2 text-xl">Transcriber</span></h1>
                                    <p className="text-slate-600 text-sm">Accurately transcribe audio files into text using Gemini 2.5.</p>
                                </div>

                                <div 
                                    className={`relative h-40 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-4 cursor-pointer transition-all hover:bg-slate-50 ${transcribeFile ? 'border-blue-500/30 bg-blue-500/5' : 'border-slate-300'}`}
                                ...
                                >
                                    ...
                                    {transcribeFile ? (
                                        <>
                                            ...
                                            <span className="text-sm font-medium text-blue-200">{transcribeFileName}</span>
                                            <button onClick={(e) => { e.stopPropagation(); setTranscribeFile(null); }} className="absolute top-4 right-4 p-2 bg-white/70 hover:bg-red-500/20 text-white/50 hover:text-red-400 rounded-full"><X size={16}/></button>
                                        </>
                                    ) : (
                                        <>
                                            <Upload size={48} className="text-slate-600" />
                                            <div className="text-center">
                                                <span className="text-sm font-bold text-slate-600">点击上传音频文件</span>
                                ...
                                <div className="flex justify-center">
                                    <button 
                                        ...
                                        className={`px-8 py-3 rounded-full font-bold text-sm shadow-lg transition-all flex items-center gap-2 ${!transcribeFile || isTranscribing ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : 'bg-blue-500 text-black hover:scale-105'}`}
                                    >
                                ...
                                {transcript && (
                                    <div className="bg-white border border-slate-300 rounded-2xl p-6 relative">
                                        <button onClick={() => navigator.clipboard.writeText(transcript)} className="absolute top-4 right-4 p-2 text-slate-500 hover:text-slate-900 bg-slate-100 hover:bg-white/70 rounded-lg transition-colors" title="复制"><FileText size={14}/></button>
                        ...
                        {activeTab === 'live' && (
                            <div className="flex flex-col items-center justify-center gap-10 py-10 animate-in fade-in slide-in-from-bottom-4 duration-300 pl-12">
                                <div className="space-y-2 text-center">
                                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">实时语音对话 <span className="text-slate-500 font-light ml-2 text-xl">Live</span></h1>
                                    <p className="text-slate-600 text-sm">Bidirectional real-time conversation with Gemini 2.0 Flash Exp.</p>
                                </div>
                                ...
                                <div className="relative w-64 h-64 flex items-center justify-center">
                                    ...
                                    {isLiveActive && (
                                        <>
                                            <div className="absolute inset-0 bg-blue-500/20 rounded-full animate-ping opacity-20" style={{ animationDuration: '2s' }} />
                                    ...
                                    {/* Status Circle */}
                                    <div className={`relative z-10 w-48 h-48 rounded-full flex flex-col items-center justify-center transition-all duration-500 border-4 ${isLiveActive ? 'bg-gradient-to-br from-cyan-900/50 to-purple-900/50 border-blue-500 shadow-[0_0_50px_rgba(6,182,212,0.3)]' : 'bg-white border-slate-300'}`}>
                                        <Activity size={48} className={isLiveActive ? "text-blue-400 animate-pulse" : "text-slate-600"} />
                                        <span className={`text-xs font-bold mt-4 uppercase tracking-widest ${isLiveActive ? 'text-blue-200' : 'text-slate-600'}`}>{liveStatus}</span>
                                    </div>
                                </div>

                                <button 
                                    onClick={toggleLive}
                                    className={`px-10 py-4 rounded-full font-bold text-base shadow-xl transition-all flex items-center gap-3 hover:scale-105 active:scale-95 ${isLiveActive ? 'bg-red-500/20 text-red-400 border border-red-500/50 hover:bg-red-500 hover:text-slate-900' : 'bg-blue-500 text-black hover:bg-cyan-400'}`}
                                >
                ...
                {(activeTab === 'factory' || (currentTrack && activeTab !== 'live')) && (
                    <div className="h-24 bg-white/95 backdrop-blur-xl border-t border-slate-200 flex items-center px-8 gap-8 relative z-20 shadow-[0_-20px_60px_rgba(15,23,42,0.08)]">
                        {/* Track Info */}
                        <div className="w-64 flex items-center gap-4">
                            <div className={`w-14 h-14 rounded-lg bg-gradient-to-br ${currentTrack ? getRandomCover(currentTrack.id) : 'from-slate-100 to-slate-200'} shadow-lg flex items-center justify-center`}>
                                {currentTrack ? <Mic2 size={24} className="text-slate-800" /> : <Disc size={24} className="text-slate-500" />}
                            </div>
                            <div className="flex flex-col min-w-0">
                                <span className="text-sm font-bold text-slate-900 truncate">{currentTrack?.title || 'Ready to speak'}</span>
                        ...
                        <div className="flex-1 flex flex-col items-center gap-2 max-w-2xl mx-auto">
                            <div className="flex items-center gap-6">
                                <button onClick={togglePlay} disabled={!currentTrack} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${currentTrack ? 'bg-white text-black hover:scale-110' : 'bg-slate-100 text-slate-600'}`}>
                                    {isPlaying ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" className="ml-0.5" />}
                                </button>
                            </div>
                            <div className="w-full flex items-center gap-3 text-[10px] font-mono text-slate-500">
                                <span>{Math.floor(currentTime / 60)}:{Math.floor(currentTime % 60).toString().padStart(2, '0')}</span>
                                <div className="flex-1 h-1 bg-slate-100 rounded-full relative overflow-hidden group/bar cursor-pointer">
                        ...
                        <div className="w-64 flex items-center justify-end gap-4">
                            <div className="flex items-center gap-2 group/vol">
                                ...
                                <div className="w-20 h-1 bg-slate-100 rounded-full relative cursor-pointer">
                                    <div className="absolute top-0 left-0 h-full bg-slate-400 group-hover/vol:bg-white rounded-full" style={{ width: `${volume * 100}%` }} />
                                </div>
                            </div>
                            <button className="p-2 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 transition-colors"><Download size={18} /></button>
                        </div>
