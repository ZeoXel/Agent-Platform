...
export const SmartSequenceDock: React.FC<SmartSequenceDockProps> = ({ isOpen, onClose, onGenerate, onConnectStart }) => {
    ...
    const renderPlayer = () => (
        <div className={`
            relative bg-white border border-slate-200 shadow-[0_25px_65px_rgba(15,23,42,0.12)] overflow-hidden flex items-center justify-center transition-all duration-500 ease-[${SPRING}]
    ...
    return (
        <>
            ...
            >
                ...
                <div 
                    className="w-full h-6 bg-white/80 backdrop-blur-md rounded-t-lg border-t border-x border-slate-200 relative overflow-hidden flex cursor-crosshair group/strip"
                ...
                >
                    {frames.map((f, i) => (
                        <div key={f.id} className="flex-1 h-full relative border-r border-slate-200 last:border-0 overflow-hidden">
                ...
                >
                    ...
                    <div className="flex-1 flex items-center gap-2 overflow-x-auto custom-scrollbar pb-1 min-h-[80px]">
                            ...
                            <React.Fragment key={frame.id}>
                                ...
                                <div 
                                    className={`
                                        ...
                                        ${draggingIndex === index ? 'opacity-30 scale-90 grayscale' : 'border-slate-300 hover:border-white/30 bg-slate-50'}
                                ...
                                >
                                    ...
                                    {/* Drag Handle (Center) */}
                                    <div 
                                        className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing bg-slate-100 backdrop-blur-[1px]"
                                        draggable
                        ...
                        {frames.length < 10 && (
                            <div 
                                className="w-[72px] h-[72px] shrink-0 rounded-lg border border-dashed border-slate-300 hover:border-white/30 bg-slate-50 hover:bg-slate-100 flex flex-col items-center justify-center gap-1 cursor-pointer transition-all group active:scale-95"
